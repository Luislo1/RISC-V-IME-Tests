...
//	scanf("%d %d %d %d", &MLEN, &KLEN, &NLEN, &seed);
	...
	sgemm_golden();
    clock_t end = clock();
    double time_golden = end - start;
    time_golden = time_golden / CLOCKS_PER_SEC;
    ...
    double time_ime = end - start;
    time_ime = time_ime / CLOCKS_PER_SEC;
	printf("sgemm_nn execution time: %lf seconds\n", time_ime);
    
    double time_diff = time_golden / time_ime;
    printf("sgemm_nn is %lf times faster than sgemm_golden\n", time_diff);
